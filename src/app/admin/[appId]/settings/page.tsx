...
export default function SettingsPage({ params }: PageProps) {
    ...
    const [appId, setAppId] = useState<Id<"apps"> | null>(null);
    ...
    useEffect(() => {
        params.then((p) => {
            setAppId(p.appId as Id<"apps">);
        });
    }, [params]);

    // Load app data
    const app = useQuery(
        api.apps.queries.getById,
        appId ? { appId } : "skip"
    );

    // Load logo and thumbnail URLs
    const logoUrl = useQuery(
        api.storage.queries.getUrl,
        app?.logoStorageId ? { storageId: app.logoStorageId } : "skip"
    );
    const thumbnailUrl = useQuery(
        api.storage.queries.getUrl,
        app?.thumbnailStorageId ? { storageId: app.thumbnailStorageId } : "skip"
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (filesToUpload.length > 0) {
                ...
                // Keep existing storage IDs if no new files uploaded
            ...
            setError(err instanceof Error ? err.message : "Failed to update app. Please try again.");
            console.error(err);
            setIsSubmitting(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : "Failed to delete app. Please try again.");
            console.error(err);
            setIsSubmitting(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-background text-primary p-8">
            <div className="max-w-2xl mx-auto">
                ...
                <div className="mb-8">
                    <button
                        onClick={() => router.push(`/admin/${appId}`)}
                    ...
                    <h1 className="text-4xl font-bold mb-3">App Settings</h1>
                ...
                <div className="mt-8 bg-surface border border-red-500/20 rounded-2xl p-8">
                    ...
                    {!showDeleteConfirm ? (
                        <button
                            ...
                            disabled={isSubmitting}
                            className="px-6 py-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl font-medium hover:bg-red-500/20 transition-all disabled:opacity-50"
                        >
                            Delete App
                        </button>
                    ) : (
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    disabled={isSubmitting}
                                    className="w-full px-4 py-3 bg-surface2 border border-red-500/20 rounded-xl text-primary placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-red-500/50 transition-all disabled:opacity-50"
                                    placeholder="DELETE"
                                />
                            </div>
                            <div className="flex gap-4">
                                <button
                                    ...
                                    disabled={isSubmitting}
                                    className="px-6 py-3 bg-surface2 border border-border rounded-xl text-secondary hover:bg-surface hover:text-primary transition-all disabled:opacity-50"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    disabled={isSubmitting || deleteConfirm !== "DELETE"}
                                    className="px-6 py-3 bg-red-500 text-white rounded-xl font-medium hover:bg-red-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
