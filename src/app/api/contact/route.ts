...
export async function POST(request: NextRequest) {
    try {
        const { name, email, message, app } = await request.json();
        ...
        // Determine subject based on app selection
        let subject = "Neue Kontakt Anfrage!";
        if (app) {
            const appNames: { [key: string]: string } = {
                memolib: "MemoLib",
                keevio: "Keevio",
            };
            subject = `Neue Kontakt Anfrage - ${appNames[app] || app}`;
        }
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd; color: #888; font-size: 12px;">
            <p>Diese Nachricht wurde über das Kontaktformular auf northbyte.studio gesendet.</p>
          </div>
        </div>
      `,
        });
