...
// Für serverseitige Aufrufe (z.B. Cloud-Run-Services) ohne Clerk-Session.
// Abgesichert über INTERNAL_API_SECRET aus der Convex-Deployment-Env.
export const createFromService = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!secret || args.internalKey !== secret) {
        ...
        let createdBy = account.assignedTo;
        if (!createdBy) {
            const admin = await ctx.db
                .query("users")
                .filter((q) => q.eq(q.field("type"), "admin"))
                .first();
